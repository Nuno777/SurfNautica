...
<body>
            ...
            <form id="contacto" action="contacto.php" method="POST" role="form" class="php-email-form" enctype="multipart/form-data">
              <div class="row">
                ...
                if (isset($_SESSION['authenticated'])) {
                  $query = "SELECT nome,email FROM users WHERE email = '{$_SESSION['email']}'";
                ...
                ?>
                  <div class="col-md-6 form-group">
                    <input type="text" class="form-control" name="nome" id="nome" placeholder="Nome" value="<?= $nome ?>" maxlength="30" readonly required>
                  </div>

                  <div class="col-md-6 form-group mt-3 mt-md-0">
                    <input type="email" class="form-control" name="email" id="email" placeholder="Email" value="<?= $email ?>" pattern="^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*\.(([0-9]{1,3})|([a-zA-Z]{2,3})|(aero|coop|info|museum|name))$" readonly required>
                  </div>
              ...
              if (isset($_SESSION['authenticated'])) {
              ?>
                <div class="text-center"><button type="submit" name="newcontato" value="newcontato">Enviar</button></div>
              <?php
              } else {
              ?>
                <div class="text-center" onclick="exibirAlerta()"><button type="submit">Enviar</button></div>
              <?php
              }
              ?>
            </form>
  ...
  <script>
    function exibirAlerta() {
      alert("Só pode enviar uma mensagem se tiver o login feito!");
    }
  </script>
</body>
