...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // validar variáveis
    if ($nome == "" || $desc == "" || empty($_FILES['inputImg']['tmp_name'])) {
        $msg_erro = "Nome, descrição ou imagem não foram inseridos!";
    } else {
        $img_tmp = $_FILES['inputImg']['tmp_name'];
        $upload_directory = '../assents/img/'; // caminho da pasta onde salva a imagem

        // Verificar se é uma imagem válida
        $check = getimagesize($img_tmp);
        if ($check !== false) {
            // Gerar um nome único para a imagem
            $img = uniqid() . '.' . pathinfo($_FILES["inputImg"]["name"], PATHINFO_EXTENSION);
            $target_file = $upload_directory . $img;

            // Verificar se o arquivo já existe
            if (file_exists($target_file)) {
                $msg_erro = "O arquivo já existe.";
            } else {
                // Mover o arquivo para o diretório de upload
                if (move_uploaded_file($img_tmp, $target_file)) {
                    $_SESSION['uploaded_image'] = $img;
                    ...
                    if ($conn->connect_error) {
                        $code = $conn->connect_error;
                        $message = $conn->connect_error;
                        $msg_erro = "Falha na ligação à Base de Dados ($code $message)!";
                    } else {
                        // Executar a query de inserção
                        $testeimage = "imagem3.jpg";
                        $query = "INSERT INTO parcerias (nome, descricao, img) VALUES ('" . $nome . "', '" . $desc . "', '" . $testeimage . "');";
                        echo $query;
            ...
            $msg_erro = "O arquivo não é uma imagem válida.";
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  $query = "UPDATE parcerias SET nome = '" . $name . "', descricao = '" . $desc . "', img = '" . $img . "' WHERE id_parceria = " . $id_parceria . ";";
  echo $query;
  $sucesso_query = mysqli_query($conn, $query);
  if ($sucesso_query) {
    if ($conn->affected_rows > 0) {
            ...
            <div class="col-lg-12 mt-5">
              ...
              <div class="card">
                <div class="card-body">
                  ...
                  <br>
                  <form action="editepartners.php" method="POST">
